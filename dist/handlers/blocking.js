...
export async function handleBlockAgent(request, env, agent) {
    try {
...
export async function handleUnblockAgent(request, env, agent) {
    try {
        const body = await request.json();
        const target = await getAgentByName(env, body.name);
        if (!target) {
...
export async function handleGetBlocked(env, agent) {
    const blocked = [];
