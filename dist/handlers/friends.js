...
export async function handleAddFriend(request, env, agent) {
    try {
...
export async function handleAcceptFriend(request, env, agent) {
    try {
        const body = await request.json();
        const friend = await getAgentByName(env, body.name);
        if (!friend) {
...
export async function handleGetFriends(env, agent) {
    const friends = [];
...
export async function handleGetPendingFriends(env, agent) {
    const pending = [];
