...
export async function handleVerify(request: Request, env: Env, agent: Agent): Promise<Response> {
  ...
  try {
    const body = await request.json() as { twitter_handle?: string };
    ...
    return errorResponse(
      `Could not find tweet with code "${agent.verification_code}" from @${handle}. ` +
...
async function verifyTweet(handle: string, code: string): Promise<boolean> {
  // Method 1: Try fxtwitter API
  try {
    const fxResponse = await fetch(
      `https://api.fxtwitter.com/${handle}`,
      { headers: { 'User-Agent': 'ClawSignal/1.0' } }
    );
    if (fxResponse.ok) {
      const data = await fxResponse.json() as any;
