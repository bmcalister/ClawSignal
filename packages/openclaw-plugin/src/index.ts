...
interface ClawSignalMessage {
  type: "message" | "ping" | "connected";
  id?: string;
  from?: string;
  message?: string;
  timestamp?: number;
...
interface ResolvedClawSignalAccount {
  accountId: string;
  enabled: boolean;
  configured: boolean;
  apiKey: string | null;
...
function getRuntime() {
  if (!pluginRuntime) throw new Error("ClawSignal runtime not initialized");
  return pluginRuntime;
}

// Config helpers
function resolveClawSignalAccount(cfg: OpenClawConfig, accountId?: string): ResolvedClawSignalAccount {
...
async function sendToClawSignal(to: string, message: string, account: ResolvedClawSignalAccount): Promise<boolean> {
...
const clawsignalChannel = {
  ...
  config: {
    ...
    resolveAccount: (cfg: OpenClawConfig, accountId?: string) =>
      resolveClawSignalAccount(cfg, accountId),
    defaultAccountId: () => "default",
    isConfigured: (account: ResolvedClawSignalAccount) => account.configured,
    describeAccount: (account: ResolvedClawSignalAccount) => ({
  ...
  status: {
    ...
    buildAccountSnapshot: ({ account, runtime }: { account: ResolvedClawSignalAccount; runtime?: any }) => ({
      accountId: account.accountId,
      enabled: account.enabled,
      configured: account.configured,
  ...
  gateway: {
    startAccount: async (ctx: any) => {
      const account = ctx.account as ResolvedClawSignalAccount;
      const runtime = getRuntime();
      const cfg = ctx.cfg;
...
`;
      ...
      const handleInboundMessage = async (from: string, text: string, fromOwner: boolean = false) => {
      ...
      const connect = () => {
        ...
        try {
          ...
          ws.on("message", async (rawData) => {
            try {
              const data = JSON.parse(rawData.toString()) as ClawSignalMessage;
...
const plugin = {
  ...
  register(api: any) {
    ...
    // Register status RPC
    api.registerGatewayMethod("clawsignal.status", ({ respond }: { respond: (ok: boolean, data: any) => void }) => {
      const runtime = getRuntime();
      const cfg = runtime.config.loadConfig() as OpenClawConfig;
      const account = resolveClawSignalAccount(cfg);
